...
export default function Pulseiras(props) {
      ...
      <main className="flex flex-1 h-full my-5 mx-5 justify-center items-center flex-col">
        ...
        <form className="flex flex-col w-full h-fit p-2 items-center mb-10" onSubmit={async (e) => {e.preventDefault(); await handleSubmit()}}>
          ...
          <div>
            ...
            <input required type={"tel"} placeholder="EM CM:" value={medida} onChange={(e)=>{e.preventDefault();setMedida(e.currentTarget.value)}} className="bg-[#D9D9D9] outline-none w-72 mb-5 font-bold text-sm justify-center items-center shadow-md border-none shadow-neutral-800 rounded-xl p-2 whitespace-nowrap " />
          ...
          <div className="w-full flex flex-col items-center">
            <div className="bg-[#B6B6B6] w-72 font-bold text-center text-sm justify-center items-center shadow-lg shadow-[#B6B6B6] rounded-xl p-2 my-5 whitespace-nowrap ">QUAL O NOME DO CLIENTE<br/>E<br/> DATA DE ENTREGA</div>
            <input required type={"text"} placeholder="Nome Do Cliente:" value={nomeCliente} onChange={(e)=>{e.preventDefault();setNomeCliente(e.currentTarget.value)}} className="bg-[#D9D9D9] outline-none w-72 mb-5 font-bold text-sm justify-center items-center shadow-md border-none shadow-neutral-800 rounded-xl p-2 whitespace-nowrap h-10" />
            <input required type={"date"} placeholder="Data De Entrega:" value={dataEntrega} onChange={(e)=>{e.preventDefault();setDataEntrega(e.currentTarget.value)}} className="bg-[#D9D9D9] outline-none w-72 mb-5 font-bold text-sm justify-center items-center shadow-md border-none shadow-neutral-800 rounded-xl p-2 whitespace-nowrap h-10" />
          </div>
          <button type="submit" className="bg-black w-72 text-white font-bold rounded-xl p-2 mt-3">FINALIZE SUA ENCOMENDA AGORA<br/>COM 1 CLIQUE</button>
        </form>
